...
const Planets = () => {
	...
	const [mouseOverLike, setHoveredFavorites] = useState({});
	
	const handlerMouseOverLike = (index) => {
		setHoveredFavorites({ ...mouseOverLike, [index]: true });
	}

	const handlerMouseLeaveLike = (index) => {
		setHoveredFavorites({ ...mouseOverLike, [index]: false });
	}
				...
				{store.planets && store.planets.length > 0 && store.planets.map((item, index) => (
					...
					<div  className="cards mx-5">
						...
						<div onMouseOver={()=>handlerMouseOverLike(index)} onMouseLeave={()=>handlerMouseLeaveLike(index)} className="card mb-5" style={{ width: "20rem" }}>
							<img src={`https://starwars-visualguide.com/assets/img/planets/${item.url.split("/")[5]}.jpg`} className={mouseOverLike[index] ? "card-img-top glow" : "card-img-top" } alt="..." onError={(e) => {
                            e.target.src = imageToLoadFail
                        }}/>
							<div className={mouseOverLike[index] ? "card-body glow" : "card-body"}>
